...
import 'UserTotalAmountScreen.dart';

class ManagementUserTotalAmount extends StatefulWidget {
  const ManagementUserTotalAmount({super.key});

  @override
  State<ManagementUserTotalAmount> createState() => _ManagementUserTotalAmountState();
}

class _ManagementUserTotalAmountState extends State<ManagementUserTotalAmount> {
  List<dynamic> users = [];
  bool isLoading = true;
  ...
  Future<void> fetchUsers() async {
    final url = Uri.parse('http://192.168.0.108:8000/api/users/with/completed/appointments/all/histories');
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          users = jsonBody['users'] ?? []; // ✅ Lấy đúng 'users' thay vì 'data'
          isLoading = false;
        });
      } else {
    ...
    return InkWell(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => UserTotalAmountScreen(
              userId: user['UserID'],
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    // Trạng thái hiển thị cố định
                    Text('Trạng thái: Đã hoàn thành', style: const TextStyle(color: Colors.black87)),
                    Row(
                      children: [
